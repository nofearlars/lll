...
import onlineSetting from './settings/onlineSetting'
import NodeConfig from './settings/NodeConfig'
import { dingtalk, email, wechat, note } from './settings/message'
...
const createStyleElement = url => {
  ...
  style.type = 'text/css'
  style.innerHTML = `body .setGuideCard{ background-image:url(${url}) }`
...
export default app.use({
  ...
  context: app => (context, cb) => {
    const { store } = context
    api({ name: 'core/setting' }).fetch('')
      .then(({ json: settings }) => {
        if (settings) {
          store.dispatch({ type: '@@iot/SAVE_SETTINGS', payload: settings, success: true })
          settings.backgroundImage && createStyleElement(settings.backgroundImage)
          settings.logo && createTabIconElement(settings.logo)
          document.title = (settings?.name || 'Air IoT物联网平台').replace(/<[^>]*>/ig, "")
          cb(null, { ...context, settings })
        } else {
          cb(null, context)
        }
      })
      .catch(() => cb(null, context))
  },
  config: {
