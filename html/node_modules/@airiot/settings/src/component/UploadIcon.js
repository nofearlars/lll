...
class BaseUpLoad extends React.Component {
  ...
  render() {
    ...
    const { imgList, isDelete, iconType } = this.props
    const iconSize = 35
    const listStyle = {
      width: 50,
      height: 50,
...
class UploadIcon extends React.Component {
  ...
  render() {
    const { t: _t1 } = useI18n()
    const { value, onChange, imgList } = this.props
    ...
    return (
      <div className="select-system-icon">
        ...
        >
          <BaseUpLoad onChange={onChange} handleCancel={this.handleCancel} imgList={imgList} isDelete={isDelete} iconType={this.props.iconType} />
        </Modal>
      </div>
    )
  }
