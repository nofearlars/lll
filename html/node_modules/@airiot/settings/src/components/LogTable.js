...
const LogTable = props => {
  const { consumerNode, consumerModel, limit, isShowPage, showlist, scrollY } = props
      ...
      .then(({ json })=>{
        if(id) {
          const newData = json&&json.map( (item,index) => {
            return {
              key: index+1,
              time: item&&new Date(item.time).getFullYear() + '-' + (new Date(item.time).getMonth() + 1) + '-' + new Date(item.time).getDate() + ' ' + new Date(item.time).getHours() + ':' + new Date(item.time).getMinutes() + ':' + new Date(item.time).getSeconds(),
